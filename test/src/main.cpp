...
#include "anonymouslib_cunroll.h"
...
int call_anonymouslib(int m, int n, int nnzA,
                  ...
                  VALUE_TYPE *x, VALUE_TYPE *y, VALUE_TYPE alpha, VALUE_TYPE beta)
{
    int err = 0;
    VALUE_TYPE *y_initial = (VALUE_TYPE *)malloc(m * sizeof(VALUE_TYPE));
    memcpy(y_initial, y, m * sizeof(VALUE_TYPE));
...
{    
    ...
    for (int i = 0; i < 50; i++) {
        if (beta != 0.0) {
            memcpy(y, y_initial, m * sizeof(VALUE_TYPE));
        } else {
    ...
    for (int i = 0; i < NUM_RUN; i++) {
        // 数据准备 - 不计时
        if (beta != 0.0) {
            memcpy(y_bench, y_initial, m * sizeof(VALUE_TYPE));
        } else {
            memset(y_bench, 0, sizeof(VALUE_TYPE) * m);
        }
        
        // 只对SpMV计时
        CSR5Spmv_timer.start();
        err = A.spmv(alpha, beta, y_bench);
        total_spmv_time += CSR5Spmv_timer.stop();
    }

    double CSR5Spmv_time = total_spmv_time / (double)NUM_RUN;
...
int main(int argc, char ** argv)
{
    ...
    //ex: ./spmv webbase-1M.mtx [alpha] [beta]
    int argi = 1;
    ...
    {
        cout << "Usage: " << argv[0] << " <matrix_file> [alpha] [beta]" << endl;
        cout << "Default: alpha=1.0, beta=0.0" << endl;
        return -1;
    }
    
    // 解析alpha和beta参数
    VALUE_TYPE alpha = 1.0;  // 默认值
    VALUE_TYPE beta = 0.0;   // 默认值
    ...
    cout << "--------------" << filename << "--------------" << endl;
    cout << "Parameters: alpha = " << alpha << ", beta = " << beta << endl;
    ...
    VALUE_TYPE *y_ori = (VALUE_TYPE *)_mm_malloc(m * sizeof(VALUE_TYPE), ANONYMOUSLIB_X86_CACHELINE);
    for (int i = 0; i < m; i++) {
        y[i] = rand() % 10;
        y_ref[i] = y[i];  // 确保初始值相同
        y_ori[i] = y[i];
    ...
    // launch compute
    call_anonymouslib(m, n, nnzA, csrRowPtrA, csrColIdxA, csrValA, x, y, alpha, beta);
